...
- UI Input Step Lite v1 e UI Explicit Input Lite v1 ja fecham passos locais de navegacao/ativacao de UI como report-only; Browser UI Input Preview v1 consome essa semantica apenas na Browser Demo, mantendo exports passivos.
...
Pacote recomendado executado: **UI Explicit Input Lite v1**, para separar `navigate`/`activate` de `InputIntent v1` antes de qualquer consumo interativo no Browser Demo/export.

**UI Explicit Input Lite v1** foi fechado como contrato externo de input UI explicito.

Escopo executado:

- criar `UiExplicitInput v1` com `navigate previous|next` e `activate`;
- criar `UiExplicitInputStepReport v1` como superficie paralela ao step legado baseado em `InputIntent v1`;
- expor runtime, CLI e MCP para validacao, traducao de teclado e inspecao de step;
...
Proximo pacote recomendado: **Audio v1 de jogo pequeno**, para evoluir `Audio Lite v1` com bancos/clips/eventos pequenos, feedback de menu/gameplay e garantia de Browser Demo/export sem dependencia externa surpresa.
