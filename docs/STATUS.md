...
- UI Input Step Lite v1 como `UiInputStepReport v1` opt-in/report-only derivado de `InputIntent v1`, `UiActionSemanticsReport v1` e `UiLocalScreenStateReport v1`, sem consumo interativo naquele slice;
- UI Explicit Input Lite v1 como `UiExplicitInput v1` externo e `UiExplicitInputStepReport v1` report-only, separando `navigate`/`activate` de `InputIntent v1`, com runtime/CLI/MCP e sem consumo interativo naquele slice;
...
- Browser UI Input Preview v1 como consumo local opt-in de `navigate`/`activate` so na Browser Demo, com `--ui-system --ui-input-preview`, MCP `uiInputPreview`, budget compacto e exports passivos;
- Browser UI Input Preview Hardening Matrix v1 como consolidacao de drift/budget/paridade do preview, com cross-interface dedicado para preview on/off, exports `ui-action-semantics` passivos em runtime/CLI/MCP e rejeicao explicita de `--ui-input-preview` nos exports CLI;
...
- tratar `Browser UI Input Preview v1`, `UiInputStepReport v1` ou `UiExplicitInputStepReport v1` como runtime interativo canonico ou export interativo;
