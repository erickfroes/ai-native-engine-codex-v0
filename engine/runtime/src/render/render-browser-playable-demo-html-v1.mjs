...
export function renderBrowserPlayableDemoHtmlV1({ title, renderSnapshot, metadata = {} }) {
  ...
  return [
    ...
    '    </header>',
    '    <section class="frame">',
    `      <p class="hint">Click the canvas, then use Arrow Keys or WASD to move the highlighted rectangle by ${stepPx} px per keydown. Pause redraw stops the local redraw loop. Reset restores the snapshot position.</p>`,
    '      <div class="controls">',
    '        <button id="browser-playable-demo-pause" type="button">Pause redraw</button>',
    ...
    '        pauseButton.textContent = redrawLoopPaused ? "Resume redraw" : "Pause redraw";',
    '      }',
